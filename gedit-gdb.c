...
static void close_gdb_session(GeditGdbPlugin *self)
{
	// 1. Stäng I/O-kanalen (detta tar även bort watch-funktionen automatiskt)
	if (self->gdb_out_chan != NULL)
	{
		g_io_channel_shutdown(self->gdb_out_chan, TRUE, NULL);
		g_io_channel_unref(self->gdb_out_chan);
		self->gdb_out_chan = NULL;
	}

	// 2. Stäng filbeskrivarna (pipes)
	if (self->gdb_stdin > 0)
	{
		close(self->gdb_stdin);
		self->gdb_stdin = 0;
	}
	if (self->gdb_stdout > 0)
	{
		close(self->gdb_stdout);
		self->gdb_stdout = 0;
	}

	// 3. Hantera process-ID och rensa resurser i operativsystemet
	if (self->gdb_pid != 0)
	{
	...
	self->connected=GEDIT_GDB_CONNECTION_STATUS_NOT_CONNECTED;

	g_print("GDB-sessionen har avslutats och resurserna har rensats.\n");
}

G_DEFINE_DYNAMIC_TYPE_EXTENDED(GeditGdbPlugin, gedit_gdb_plugin, G_TYPE_OBJECT, 0,
                               G_IMPLEMENT_INTERFACE_DYNAMIC(GEDIT_TYPE_WINDOW_ACTIVATABLE, gedit_gdb_plugin_window_activatable_iface_init))
...
static void send_gdb_command(GeditGdbPlugin *self, const gchar *command, gboolean force)
{
	if (self->gdb_stdin > 0 && (self->connected==GEDIT_GDB_CONNECTION_STATUS_CONNECTED || force))
	{
...
static gboolean on_gdb_output(GIOChannel *source, GIOCondition cond, gpointer data)
{
	...
	if (g_io_channel_read_line(source, &line, NULL, NULL, NULL) == G_IO_STATUS_NORMAL)
	{
		...
		else if (g_str_has_prefix(line, "^connected"))
		{
			self->connected=GEDIT_GDB_CONNECTION_STATUS_CONNECTED;
			send_gdb_command(self, "-break-insert main",FALSE);
			send_gdb_command(self, "-exec-continue",FALSE);
		}
		else if (g_str_has_prefix(line, "=thread-group-exited"))
		{
			const gchar *exit_code = strstr(line, "exit-code=\"");

			int exit_code_int = 0;

			sscanf(exit_code, "exit-code=\"%d\"", &exit_code_int);

			GtkWidget *dialog;

			dialog = gtk_message_dialog_new(GTK_WINDOW(self->window), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
			                                GTK_BUTTONS_OK, "Exit code %d", exit_code_int);

			gtk_window_set_title(GTK_WINDOW(dialog), "Information");

			// Kör dialogen synkront (blockerar tills användaren trycker OK)
			gtk_dialog_run(GTK_DIALOG(dialog));

			// Förstör dialogen för att rensa minnet
			gtk_widget_destroy(dialog);

			close_gdb_session(self);
		}
		else if (g_str_has_prefix(line, "^error,msg=\"The program is not being run.\"") || g_str_has_prefix(line, "^error,msg=\"could not connect"))
		{
			close_gdb_session(self);
		}
	}
...
static void gdb_connect_to(GeditGdbPlugin *self, const char *const location)
{
	if(self->connected==GEDIT_GDB_CONNECTION_STATUS_NOT_CONNECTED)
	{
		gchar *argv[] = {"/usr/bin/gdb", "--interpreter=mi", NULL};
		if (g_spawn_async_with_pipes(NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &self->gdb_pid, &self->gdb_stdin,
					                 &self->gdb_stdout, NULL, NULL))
		{

			self->gdb_out_chan = g_io_channel_unix_new(self->gdb_stdout);
			g_io_add_watch(self->gdb_out_chan, G_IO_IN, on_gdb_output, self);
			
			g_autofree gchar *command = g_strdup_printf("-target-select remote %s", location);
			
			send_gdb_command(self, command, TRUE);
			self->connected=GEDIT_GDB_CONNECTION_STATUS_CONNECTING;
		}
		else
		{
...
{
	send_gdb_command(self, "-exec-step",FALSE);
}

static void on_next_clicked(GtkButton *btn, GeditGdbPlugin *self)
{
	send_gdb_command(self, "-exec-next",FALSE);
}

static void on_up_clicked(GtkButton *btn, GeditGdbPlugin *self)
{
	send_gdb_command(self, "-interpreter-exec console \"up\"",FALSE);
}

static void on_down_clicked(GtkButton *btn, GeditGdbPlugin *self)
{
	send_gdb_command(self, "-interpreter-exec console \"down\"",FALSE);
}
...
static void on_break_clicked(GtkButton *btn, GeditGdbPlugin *self)
{
	...
	// Skapa en dialog
	dialog = gtk_dialog_new_with_buttons("Sätt brytpunkt", GTK_WINDOW(self->window), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
	                                     "_Avbryt", GTK_RESPONSE_CANCEL, "_Sätt brytpunkt", GTK_RESPONSE_OK, NULL);
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK)
	{
		...
		if (location && strlen(location) > 0)
		{
			g_autofree gchar *command = g_strdup_printf("-break-insert %s", location);
			send_gdb_command(self, command,FALSE);
...
static void gedit_gdb_plugin_activate(GeditWindowActivatable *activatable)
{
	...
	GtkWidget *btn_continue = gtk_button_new_with_label("Continue");
	GtkWidget *btn_break = gtk_button_new_with_label("Break");
	...
	if (side_panel != NULL)
	{
		...
		TeplPanelItem *item = tepl_panel_item_new(self->panel, "gedit-gdb-debugger", "GDB Debugger", "builder-debugger-symbolic", -1);
...
static void gedit_gdb_plugin_deactivate(GeditWindowActivatable *activatable)
{
	...
	if (self->gdb_pid)
	{
		send_gdb_command(self, "-gdb-exit",FALSE);
		g_spawn_close_pid(self->gdb_pid);
	}
	if (self->panel)
	{
